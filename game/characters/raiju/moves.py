...
def make_raiju_abilities():
    return {
        ...
        "skills": [
            ...
            # burning for 5s.
            Ability("Chain Bolt", "skill", "homing_bolt", 4, 1.5, tag="chain_bolt", ignore_clone=True),
            ...
            # tracking).
            Ability("Static Field", "skill", "cast", 5, 0.0, tag="static_field", ignore_clone=True,
                    cast_target="enemy"),
            ...
            Ability("Static Link", "skill", "cast", 4, 0.0, tag="static_link",
                    one_shot=True, ignore_clone=True, cast_target="self"),
        ],
        # Calls down a single sky-splitting bolt: the longest stun and burn
        # in Raiju's kit. No ignore_clone: aoe_radius alone already means
        # this blast damages every body inside it rather than picking one to
        # land on (see StatusLibraryMixin.taunt_redirect).
        "ultimate": Ability("Thunder God's Descent", "ultimate", "sky_strike", 10, 2.5,
                             big=True, tag="thunder_descent", aoe_radius=120),
    }
